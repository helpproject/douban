// import new rating JS from shire.
/*
try{
...
this.set_overlay = function(){
    var dialog = $('#dialog'),
        h = dialog[0].offsetHeight + ($.browser.msie?-2:16),
...
Douban.init_editable = function(o){
    ...
    var show = function(t){
        if(t != undefined){
            var html = $('<div>').text(t).html().replace(/\n/g, '<br>');
            disp.html(html);
            if (disp.text() == ''){
...
;(function() {
  ...
  var old_init_interest_form = Douban.init_interest_form
  Douban.init_interest_form = function(form) {
    var sid = $(form).attr('action').split('/')[3];
    var interests = $(form).find('input[name="interest"]')
    $(form).submit(function() {
      trackEvent('collection'
        , (interests.filter(':checked')[0] || interests[0]).value
        , sid)
    })
    return old_init_interest_form(form)
  }

  var old_init_collect_btn = Douban.init_collect_btn
  Douban.init_collect_btn = function(btn) {
    var sid = $(btn).attr('name').split('-')[1]
    $(btn).click(function() {
      trackEvent('collection', 'try', sid)
    })
    return old_init_collect_btn(btn)
  }
})();
