(function (global) {
    ...
    var adUnits = global.DoubanAdSlots.filter(function(n){
    ...
    var loadFrame = function(query, parameters) {
        ...
        var unit = query.unit || '',
            ...
            adSlot = global.document.getElementById(el),
            adFrame = global.document.getElementById(el+'_frame');
            adSlot.setAttribute('data-id', (parameters['ad_id'] || ''));
    ...
    global.DoubanAdGet = function(slots, loaders, adElements) {
        ...
        for (var i = 0,elObj,el=''; i<slots.length; i++) {
            if (adElements && adElements[i]) {
                elObj = adElements[i];
                el = elObj['el'] || '';
                var adid = elObj['id'];
                slot = global.document.getElementById(el);
                adid = adid || slot.getAttribute('data-id') || '';
                if (adid) {
                    adids.push(adid);
                }
                adidstr = adids.join(',');
    ...
    global.DoubanAdGet(global.DoubanAdSlots, global.DoubanAdLoaders, global.adElements);
})(this);
