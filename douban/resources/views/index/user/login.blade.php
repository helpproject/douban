...
<div class="wrapper">
    ...
    <div id="content">
        ...
        <div class="article">
            ...
            @endif
            <form id="lzform"  method="post" onsubmit="return validateForm(this);" action="/login">
                <div style="display:none;">
                    <img src="Picture/blank.gif" onerror="document.lzform.action='https://accounts.douban.com/login'"/>
                </div>
                <input name="source" type="hidden" value="book"/>
                <input name="redir" type="hidden" value="https://www.douban.com"/>
                <div class="item-right">
                    <a href="?redir=http://www.douban.com&amp;source=book&amp;login_type=sms">手机验证码登录</a>
                </div>
                <div class="item">
                    <label>帐号</label>
                    <input id="email" name="username" type="text" class="basic-input"
        ...
        <ul id="side-nav" class="aside">
            <li>&gt;&nbsp;还没有豆瓣帐号？<a rel="nofollow" href="/register">立即注册</a></li>
        </ul>
    </div>
    <div id="footer">
...
<span class="fright">
    ...
    <script>
        ...
        (function (e) {
            ...
            var f = "邮箱/手机号/用户名";
            var b = d("email"), a = d("password"), c = d("captcha_field");
            b.onfocus = function () {
                if (this.value == f) {
                    this.value = "";
        ...
        function validateForm(f) {
            var c = 0, b = f.elements["captcha-solution"], e = f.elements.form_email, a = f.elements.form_password, h = document.getElementById("item-error");
            if (h) {
            ...
            if (b) {
                var g = trim(b.value);
                if (g === "") {
                    displayError(b, "请输入验证码");
                    c = 1
                } else {
                    if (g.length < 4) {
                        displayError(b, "请输入正确的验证码");
                        c = 1
                    } else {
                        clearError(b)
                    }
            ...
            if (e) {
                var d = trim(e.value);
                if (d === "" || d === "邮箱/手机号/用户名") {
                    displayError(e, "请输入正确的邮箱/手机号/用户名");
                    c = 1
                } else {
                    clearError(e)
                }
        ...
        function displayError(a, c) {
            var b = document.getElementById(a.name + "_err");
            if (!b) {
                b = document.createElement("span");
                b.id = a.name + "_err";
                b.className = "error-tip";
